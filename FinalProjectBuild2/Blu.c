...
void determinePWM(uint32_t calibratedData){
    ...
    if (PWMcontroller!=dutyCycleTotal){
        //printf("Duty cycle total in determine PWM= %d\n", dutyCycleTotal);
        //printf("PWM controller in determine PWM= %d\n", PWMcontroller);
        //printf("calibratedData from the fuckin' temp sensor= %d\n", calibratedData);

        if (calibratedData<=7900){  // hotter 2.065 2.285
            dutyCycleTotal=100;

...
void step_state(enum events event){
   ...
   case locked_state:{
       switch (events){
           ...
           //This part clears the display to prepare for the locked
           //state and waits for user to input the correct code
           LCD_command(0x01);
           Write_string_LCD("LOCKED");
           LCD_command(0xC0);
           Write_string_LCD("ENTER CODE:");

               //This while loop takes the user input and directs
               //you to the change code state if # is input
               while(indexer<4){
                   key = keypad_getkey();
                   if (indexer==0 && key==35){
               ...
               //if the size entered is equal to 4 compare the input.
               //If it matches it is unlocked. If it doesnt match it is redirected to the locked state
               if (indexer==4){
   ...
   case confirm_oldcode_state:{
       switch (events){
           case confirm_oldcode_event:
               ...
               //This checks if the user input matches the new code.
               //If it does the next state is New Code. If it doesnt, the next state is the locked state
               if (indexer==4){
                   int comparison=memcmp(compareArray,lockedArray, sizeof(compareArray));
                   if (comparison==0){
                       LCD_command(0x01);
   ...
   case newCode_state:
       switch (events){
           case newCode_event:
               ...
               //Setting up the LCD display for user input.
               //This takes in the values for the new 4-digit code.
               LCD_command(0x01);
   ...
   //Confirm new code is reached once the user inputs the initial new code.
   //If it matches it confirms new code and goes to the unlocked state
   case confirm_newcode_state:
       switch (events){
           case confirm_newcode_event:
               ...
               //This while statement checks if the user input is a vaild
               //input and displays it on the LCD
               while(indexer<4){
                   key = keypad_getkey();
                   if (key>9 && key!=255){
                       if (key==42){
               ...
               //if the size entered is equal to 4 compare the input.
               //If correct, the code will change and the unlocked state will be reached

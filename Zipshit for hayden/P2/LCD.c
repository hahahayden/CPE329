...
void LCD_init(void) {
   ...
   delay_us(40);               /* delay can be replicated from writecom
                                * subroutine on page 11 */
...
void LCD_4bit_write(unsigned char data, unsigned char control) {
   ...
   P4->OUT = data | control;       /* RS = 0, R/W = 0;  allows the above to
                                    * fit in 4bit data 3 bit control*/
...
void delay_us(int time)
{
   ...
   int freq= 24000000;//changed from 3000000
   int clock;
   int freqMhz= freq/1000000;
   int i;
   clock=time;
   for (i=clock; i>0; i--); /* create a for loop with the appropriate number
                             * of iterations to create a delay approximate to
                             * the input */
}
void LCD_Line2(){           // allows for displaying at Line 2
   LCD_command(0xC0);
}
void clear_LCD(){           // calls to clear the display and return home
   LCD_command(0x01);
}
void Home_LCD(){            // calls to return to Line1 (zeroth position)
   LCD_command(0x80);
}
